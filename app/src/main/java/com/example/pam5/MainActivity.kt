...
import com.example.pam5.Data.DataSource.sttus
...
fun TampilForm(cobaViewModel: CobaViewModel= viewModel()){
    ...
    SelectJK(options = jenis.map { id -> context.resources.getString(id) },
        onSelectionChanged = { cobaViewModel.setjenisKl(it)})
    SelectSTS(options = sttus.map { id -> context.resources.getString(id) },
    ...
    Button(
        modifier = Modifier.fillMaxWidth(),
        onClick = { cobaViewModel.insertData(textNama,textTlp, texteml, dataFrom.sex, dataFrom.stts, textAlmt )})
...
@Composable
fun TextHasil(emailnya: String, jenisnya:String, statusnya : String, alamatnya : String) {
    ElevatedCard (
